    ...
    return (
        <div className={`row g-4 mb-4 ${styles.comment}`}>
            <div className='col-12 d-flex align-items-center'>
                ...
                {!editingMode &&    <>
                    <div className='flex-shrink-0'>
                        <img
                            ...
                            src={comment.author && comment.author.image ? comment.author.image : 'https://facit-modern.omtanke.studio/static/media/wanna2.35c7aa82595ff8db5ef4.png'}
                            alt='Avatar'
                            width='64'
                            height='64'
                        />
                    ...
                    <div className='flex-grow-1 ms-3 d-flex justify-content-between align-items-center'>
                        <figure className='mb-0'>
                            <blockquote className='blockquote' dangerouslySetInnerHTML={{__html: comment.text}}/>
                            <figcaption className='blockquote-footer mb-0'>
                                {comment.author.name} {comment.author.second_name}
                                {comment.author.city && <> from <cite>{comment.author.city}</cite></>}
                            </figcaption>
                        </figure>
                    </div>
                </>}
                ...
                {
                    editingMode && <div className='col-12'>
                        <Textarea placeholder='Write your text here'
                                  className='d-block'
                                  size='lg'
                                  rows={10}
                                  value={updatedComment}
                                  onInput={(ev:any) => setUpdatedComment(ev.target && ev.target?.value ? ev.target?.value : '')}/>
                        <ButtonGroup className='mt-5'>
                            <Button color='primary'
                                    icon='Message' size='lg'
                                    onClick={saveComment}
                                    isDisable={updatedComment.trim().length < 3}>Save comment</Button>
                            <Button color='danger'
                                    icon='Cancel' size='lg'
