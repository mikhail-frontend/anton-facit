...
const CourseContent:React.FC<CourseContentType> = ({ showLessons}) => {
    const currentCourse:currentCourseType = useSelector((state: any) => state.courseItem.currentCourse);
    ...
    return (
        <>
            ...
            {showLessons && <div className='row mb-5'>
                <div className='col-12'>
                    <h2 className='mt-4 mb-2'>Course lessons</h2>
                </div>
                {currentCourse.lessons.map(lesson => {
                    return (
                        <CourseLesson key={lesson.id}
                                      id={lesson.id}
